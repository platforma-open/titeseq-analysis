...
def test_poisson_titeseq_kd_recovery():
    cfg = PoissonConfig(
        true_kd=10.0, true_n=1.0,
        baseline=1.0, amplitude=math.log(2.5),
        concs=[0.1, 0.3, 1.0, 3.0, 10.0, 30.0, 100.0, 300.0],
        n_clonotypes=50, n_bins=4, reads_per_conc=2000, seed=42,
    )
    ...
    within = good.filter(
        (pl.col("kd") >= cfg.true_kd / 5) & (pl.col("kd") <= cfg.true_kd * 5)
    )
