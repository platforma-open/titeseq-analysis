...
import { TiteseqAnalysisBlockPointer as myBlockSpec } from "this-block";
...
blockTest(
  ...
  async ({ rawPrj: project, ml, helpers, expect }) => {
    ...
    const doneState = await helpers.awaitBlockDoneAndGetStableBlockState(titeseqBlockId, 360000);
    ...
    if (!tableHandle)
      throw new Error("summaryTable.fullTableHandle unexpectedly undefined after block ran");
    ...
    const tableData = await ml.driverKit.pFrameDriver.getData(tableHandle, [
      affinityIdx,
      kdIdx,
      r2Idx,
    ]);
    ...
    expect(
      affinityCounts.Good,
      `Good count (distribution: ${JSON.stringify(affinityCounts)})`,
    ).toBeGreaterThanOrEqual(1);
    expect(
      affinityCounts.Failed,
      `Failed count (distribution: ${JSON.stringify(affinityCounts)})`,
    ).toBeGreaterThanOrEqual(1);
    ...
    const inRange = goodKds.filter((kd) => kd >= 3e-10 && kd <= 3e-8);
    expect(
      inRange.length,
      `Kd in expected range (Good Kds: ${goodKds.join(", ")})`,
    ).toBeGreaterThanOrEqual(1);
    ...
    if (!titrationCurvesPf)
      throw new Error("titrationCurvesPf unexpectedly undefined after block ran");
