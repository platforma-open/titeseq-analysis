...
blockTest(
  'multi-instance trace labels are distinct (antigen-mode fixture)',
  { timeout: 900000 },
  async ({ rawPrj: project, helpers, expect }) => {
    // Regression test for the "TiteSeq labels machinery is not implemented" bug.
    // Two TiteSeq Analysis blocks on the same upstream chain, distinct
    // targetAntigen — assert their emitted summary columns carry distinct
    // `pl7.app/trace` annotations, so the platform's deriveLabels() algorithm
    // can disambiguate them downstream (e.g. in Lead Selection's filter dropdown).
    const sndBlockId = await project.addBlock('Samples & Data', samplesAndDataBlockSpec);
    const importBlockId = await project.addBlock('Import V(D)J Data', importVdjBlockSpec);
    const titeseqAId = await project.addBlock('Titeseq A', myBlockSpec);
    const titeseqBId = await project.addBlock('Titeseq B', myBlockSpec);

    // Antigen fixture: 56 samples, antigen column has values "other" and "target".
    const { args: sndArgs } = await prepareSamplesAndDataArgs('antigen', helpers);
    await project.setBlockArgs(sndBlockId, sndArgs);
    await project.runBlock(sndBlockId);
    await helpers.awaitBlockDone(sndBlockId, 60000);

    const importState1 = await awaitStableState(project.getBlockState(importBlockId), 30000);
    const importOutputs1 = wrapOutputs(importState1.outputs!) as {
      datasetOptions?: { ref: PlRef; label: string }[];
    };
    const datasetRef = importOutputs1.datasetOptions![0].ref;
    ...
    // Both TiteSeq instances see the same option lists — they both feed off the
    // single Import V(D)J chain. Read once from block A and reuse for B.
    const idleAState = (await awaitStableState(
      project.getBlockState(titeseqAId),
      30000,
    )) as InferBlockState<typeof model>;
    const idleAOutputs = wrapOutputs<BlockOutputs>(idleAState.outputs);
    const abundanceRef = idleAOutputs.abundanceOptions[0].ref;
    const concentrationRef = idleAOutputs.concentrationOptions.find(
      (o) => o.label === 'antigen_conc_M',
    )!.ref;
    const binRef = idleAOutputs.binOptions.find((o) => o.label === 'bin_number')!.ref;
    const antigenRef = idleAOutputs.antigenOptions.find((o) => o.label === 'antigen')!.ref;

    // Configure both blocks identically except for the per-instance fields:
    //   - targetAntigen — drives which antigen subset the Python fit consumes
    //   - defaultBlockLabel — simulates the UI auto-derivation result; the
    //     workflow now reads (customBlockLabel || defaultBlockLabel) for the
    //     trace label, so distinct values here MUST produce distinct traces.
    const commonData = {
      abundanceRef,
      concentrationColumnRef: concentrationRef,
      binColumnRef: binRef,
      antigenColumnRef: antigenRef,
    ...
    await project.mutateBlockStorage(titeseqAId, {
      operation: 'update-block-data',
      value: {
        ...commonData,
        targetAntigen: 'other',
        defaultBlockLabel: 'other',
      } satisfies BlockData,
    });
    await project.mutateBlockStorage(titeseqBId, {
      operation: 'update-block-data',
      value: {
        ...commonData,
        targetAntigen: 'target',
        defaultBlockLabel: 'target',
      } satisfies BlockData,
    });

    await project.runBlock(titeseqAId);
    await project.runBlock(titeseqBId);
    const doneAState = await helpers.awaitBlockDoneAndGetStableBlockState(titeseqAId, 360000);
    const doneBState = await helpers.awaitBlockDoneAndGetStableBlockState(titeseqBId, 360000);

    const outA = wrapOutputs<BlockOutputs>(doneAState.outputs as unknown as BlockOutputs);
    const outB = wrapOutputs<BlockOutputs>(doneBState.outputs as unknown as BlockOutputs);

    const affinityA = outA.summaryPfCols?.find(
      (c) => c.spec.name === 'pl7.app/vdj/affinityClass',
    );
    const affinityB = outB.summaryPfCols?.find(
      (c) => c.spec.name === 'pl7.app/vdj/affinityClass',
    );
    expect(affinityA, 'block A affinityClass column').toBeDefined();
    expect(affinityB, 'block B affinityClass column').toBeDefined();

    const traceAStr = affinityA!.spec.annotations?.['pl7.app/trace'];
    const traceBStr = affinityB!.spec.annotations?.['pl7.app/trace'];
    expect(traceAStr, 'block A trace annotation').toBeDefined();
    expect(traceBStr, 'block B trace annotation').toBeDefined();

    // Core assertion: the full trace strings differ. deriveLabels() needs SOME
    // difference between the two columns' traces to emit distinct labels.
    expect(traceAStr).not.toEqual(traceBStr);

    // Per-instance entry: the TiteSeq own descriptor must carry the
    // defaultBlockLabel value through. If this drifts back to a single
    // hard-coded "Tite-Seq Analysis" the regression returns.
    const traceA = JSON.parse(traceAStr!) as { type: string; label: string }[];
    const traceB = JSON.parse(traceBStr!) as { type: string; label: string }[];
    const ownA = traceA.find((t) => t.type === 'milaboratories.titeseq-analysis');
    const ownB = traceB.find((t) => t.type === 'milaboratories.titeseq-analysis');
    expect(ownA?.label, 'block A own trace label').toBe('other');
    expect(ownB?.label, 'block B own trace label').toBe('target');
  },
);
